    ...
    const emailHTML = `
        <div style="width: 700px; margin: 0 auto;">
            <div style="background: #2f353a; padding: 15px 10px; text-align: left;">
                <a href="http://35.89.29.41:3000/register">
                    <img src="http://35.89.29.41:3000/assets/register/image/form-logo.png" alt="Esfaira" style="height:30px;">
                </a>
            </div>
            ...
            <div style="background: #F4F4F4; padding: 15px; text-align: center; border: #CCC 1px solid; border-top: none;">
                Powered by
                <a href="http://35.89.29.41:3000" style="color:#ffae00;">ESFAIRA</a>
            </div>
        </div>`;
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.redirect('http://35.89.29.41:3000/api/auth/invalid-token');
    }
    ...
    res.redirect('http://35.89.29.41:3000/api/auth/reset-password-success');
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error.' });
  }
